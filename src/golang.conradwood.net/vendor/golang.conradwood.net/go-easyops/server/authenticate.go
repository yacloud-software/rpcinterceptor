...
func compare_intercept_user(name string, u1, u2 *apb.User) {
	...
	if u1 == nil {
		fmt.Printf("%s %s\n", u1s, u2s)
		panic(fmt.Sprintf("[go-easyops] intercept user check \"%s\" mismatch (u1 is nil, u2 is not)", name))
	}
	if u2 == nil {
		fmt.Printf("%s %s\n", u1s, u2s)
		panic(fmt.Sprintf("[go-easyops] intercept user check \"%s\" mismatch (u2 is nil, u1 is not)", name))
	}
	if u1.ID != u2.ID {
		fmt.Printf("%s %s\n", u1s, u2s)
